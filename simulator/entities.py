...
class Planet(Entity):
    def __init__(self, weight: float, position: Point, speed: Vector, radius: float) -> None:
        super().__init__(weight, position, speed)
        self.radius = radius


class SimPlanetaryObject(Sprite):
    def __init__(self, entity: Entity, screen_coordinates, shape="circle", color=pygame.Color("White")) -> None:
        super().__init__()
        self.entity = entity
