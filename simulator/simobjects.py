...
from events import EventRegistrer, CaptureEvent
...
class SimObject(Sprite):
    ...
    def process_mouseclick(self, mousepos: Point):
        x, y = self.centre_on_screen
        if max([abs(mousepos.x - x), abs(mousepos.y - y)]) < config.CLICK_RADIUS:
            EventRegistrer.register_event(CaptureEvent(self))
...
class SimPlanetaryObject(SimObject):
    ...
    def draw(self, screen, font):
        ...
        pygame.draw.circle(
            screen, self.color, self.centre_on_screen,
